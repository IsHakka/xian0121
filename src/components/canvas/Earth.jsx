...
const Earth = () => {
  ...
  return (
    <primitive object={earth.scene} scale={3} position-y={0} position-x={0}></primitive>
  )
}
  ...
  return (
    <Canvas shadows frameLoop='demand' gl={{ preserveDrawingBuffer: true }} camera={{fov:45,near:0.1,far:200,position:[-4,3,6]}}>
      <Suspense fallback={<CanvasLoader></CanvasLoader>}>
        <OrbitControls autoRotate enableZoom={false} maxPolarAngle={Math.PI / 2} minPolarAngle={Math.PI / 2}></OrbitControls>
        <Earth></Earth>
      </Suspense>
    </Canvas>
  )
